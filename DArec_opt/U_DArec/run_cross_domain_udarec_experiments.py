#!/usr/bin/env python3
"""
U-DArec Cross-Domain Training Experiment Runner

This script runs cross-domain training experiments for U-DArec across all Amazon product categories.
Designed to be run in the background for long-running experiments.

Usage:
    python run_cross_domain_udarec_experiments.py [options]
    
    # Run in background with nohup:
    nohup python run_cross_domain_udarec_experiments.py > experiment_output.log 2>&1 &
    
    # Run with custom parameters:
    python run_cross_domain_udarec_experiments.py --epochs 100 --batch-size 32
"""
...
def setup_logging(log_dir="./logs"):
    ...
    log_file = os.path.join(log_dir, f"udarec_experiment_runner_{timestamp}.log")
...
def main():
    parser = argparse.ArgumentParser(description='Run U-DArec cross-domain training experiments')
    ...
    parser.add_argument('--weight-decay', type=float, help='Weight decay (default: 1e-4)')
    parser.add_argument('--n-factors', type=int, help='Number of latent factors (default: 200)')
    ...
    logger.info("Starting U-DArec cross-domain training experiment runner")
    ...
    try:
        if args.grid_search:
            logger.info("Running U-DArec grid search experiments")
            ...
            logger.info("Running U-DArec single configuration experiments")
        ...
        logger.info("All U-DArec experiments completed successfully!")
        if isinstance(results, list) and len(results) > 0:
            # Count successful vs failed experiments
            successful = len([r for r in results if 'error' not in r])
            failed = len([r for r in results if 'error' in r])
            logger.info(f"Summary: {successful} successful, {failed} failed experiments")
        
    except KeyboardInterrupt:
        logger.info("U-DArec experiments interrupted by user")
        return 1
    except Exception as e:
        logger.error(f"Error during U-DArec experiments: {e}")
        import traceback
