...
# New function: computes pairwise cosine distance between samples (rows) of two matrices
def cost_matrix_batch_samples_torch(x, y):
	"""
	Computes pairwise cosine distance between samples (rows) of x and y.
	x: [batch_size, n_factors]
	y: [batch_size, n_factors] (or [other_batch_size, n_factors])
	Returns: [batch_size, other_batch_size] cost matrix
	"""
...
def IPOT_torch(C, n, m, miu, nu, beta=0.5):
	...
	for t in range(20):
		...
		for k in range(1):
			...
			# Debug print shapes before the problematic matmul
			print("[DEBUG] Iteration", t, "k", k)
			...
			print("  torch.transpose(T,0,1) shape:", torch.transpose(T,0,1).shape)
			print("  torch.unsqueeze(delta,1) shape:", torch.unsqueeze(delta,1).shape)
...
# Stable version of IPOT_torch for batchwise OT with correct delta/sigma shapes
def IPOT_torch_stable(C, n, m, miu, nu, beta=0.5, n_iter=20):
    """
    C: [n, m] cost matrix
    miu: [n, 1]
    nu: [m, 1]
    ...
    """
    sigma = torch.ones(m, 1, device=C.device, dtype=C.dtype) / m
    T = torch.ones(n, m, device=C.device, dtype=C.dtype)
    C = torch.exp(-C / beta).float()
    for t in range(n_iter):
        T = C * T
        delta = miu / (torch.matmul(T, sigma) + 1e-12)  # [n, 1]
        sigma = nu / (torch.matmul(T.t(), delta) + 1e-12)  # [m, 1]
        T = delta * T * sigma.t()  # [n, m]
    return T.detach()
